...
function GuestOrderForm({ guestId, onOrderSuccess }) {
    ...
    const { orders: historyOrders } = useRealTimeOrders('guestId', guestId, [0, 7, 8], viewMode === 'ordering');
    ...
    // 🧠 AI RECOMMENDATION STREAM TRIGGER (DIRECT SERVER ENVIRONMENT EDITION)
    // 🧠 AI RECOMMENDATION STREAM TRIGGER (STABILIZED PRODUCTION EDITION)
    // Avoids race conditions and network abort actions during mount transitions
    const apiLockRef = useRef(false);

    useEffect(() => {
        let isMounted = true; 

        async function triggerSecureAIRequest() {
            // Guard clause: Exit immediately if prerequisites aren't met, or if locked/loaded
            if (
                viewMode !== 'ordering' || 
                !flatMenuItemsList || 
                flatMenuItemsList.length === 0 || 
            ...
            // Fallback placeholder dataset array if handling a brand-new guest session
            const safeHistory = historyOrders && historyOrders.length > 0 
                ? historyOrders 
                : [{ items: ["First-time Discovery Session"], orderType: "Standard" }];

            setAiLoading(true);
            apiLockRef.current = true; // Lock the API path immediately
            
            try {
                console.log(`📡 Shipping ${flatMenuItemsList.length} local items securely to /api/recommendations endpoint...`);
                const recommendedItemIds = await requestPersonalizedMealSuggestions(safeHistory, flatMenuItemsList);
                
                if (!isMounted) return;

                console.log("🧠 Server API route responded successfully. Chosen item IDs:", recommendedItemIds);

                // Map the returned food item string IDs back onto your local database details layout safely
                const matchedObjects = flatMenuItemsList.filter(item => recommendedItemIds.includes(item.id));
                
                if (matchedObjects.length > 0) {
                    setAiSuggestions(matchedObjects);
                } else {
                    console.warn("⚠️ AI returned string IDs that could not be verified within your active menu listings dataset. Received IDs:", recommendedItemIds);
                    // Reset lock on empty mismatch so it can try again if the menu stabilizes
                    apiLockRef.current = false;
                }

            } catch (error) {
                console.error("❌ Failed executing transaction route sequence:", error);
                if (isMounted) apiLockRef.current = false;
            } finally {
                if (isMounted) setAiLoading(false);
            }
        }
        
        triggerSecureAIRequest();
        
        return () => {
            isMounted = false;
            // Leave the lock intact if suggestions were found, otherwise clean up
        }; 
    // 🚀 FIXED: Isolated dependencies to prevent parallel network tracking aborts
    }, [viewMode, flatMenuItemsList?.length]);
    ...
    const placeOrder = async () => {
        ...
        try {
            ...
            // 🚀 REFACTORED WORKLOAD: Pack clean configuration properties to our action engine
            const packedOrderPayload = {
                ...
                // 💥 FIXED LINE: Send the exact dynamic state value down to the actions handler!
                serviceCharge: orderType === 'Room Service' ? Number(flatCharge) : 0.00
            };

            // Trigger the centralized transaction pipeline
    ...
    return (
          <div style={containerStyle}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', padding: '4px 0', width: '100%', boxSizing: 'border-box' }}>
                <button 
                    onClick={() => {
                        if (isMenuDetailsPhase && !isCustomOnlyMode) {
         ...
         {!isMenuDetailsPhase && !isCustomOnlyMode && (
                <div style={menuContainerFadeIn}>
                    ...
                    {/* ✨ NEW: CHEF'S AI HANDPICKED SUGGESTIONS CAROUSEL */}
                    {aiSuggestions.length > 0 && (
                        <div style={{ width: '100%', marginBottom: '32px', padding: '0 4px', boxSizing: 'border-box', animation: 'fadeSlideUp 0.5s ease-out' }}>
                            ...
                            {/* Horizontal Scroll Track (Scrollbar hidden beautifully) */}
                            <div style={{ display: 'flex', gap: '14px', overflowX: 'auto', paddingBottom: '12px', scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' }}>
                                {aiSuggestions.map(item => (
                                    ...
                                    >
                                        ...
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation(); // Prevents double-triggering the parent card click
                                                handleSelectItemForEdit(item);
                                            }}
                                            style={{
                                                marginTop: '14px',
                                                width: '100%',
                                        ...
                                        {/* Little native quantity display chip if item is already added */}
                ...
                <div style={{...sectionStyleGuestForm, animation: 'fadeSlideUp 0.4s ease-out'}}>
                    ...
                    {orderType === 'Room Service' ? (
                        <div style={{ padding: '20px', backgroundColor: '#FBFBF9', borderRadius: '20px', border: '1px solid #E8E8E1', marginBottom: '25px' }}>
                            <label style={{ ...labelStyle, color: validationErrors.room ? '#dc3545' : '#121212' }}>
                                Search & Confirm Your Room:
                            </label>
                            ...
                            <input 
                                ...
                                style={{
                                    ...inputStyle,
                                    borderColor: validationErrors.room ? '#dc3545' : '#E8E8E1',
                                    backgroundColor: selectedRoom ? '#E8F5E9' : '#FFFFFF' 
                                }} 
                            ...
                            <select 
                                ...
                                style={{...selectStyle, marginBottom: 0, marginTop: '12px', border: selectedRoom ? '1px solid #A5D6A7' : '1px solid #E2E2E2'}}
                            >
                            ...
                            {selectedRoom && (
                                <div style={{ fontSize: '0.75rem', color: '#2E7D32', fontWeight: '700', marginTop: '8px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                    ✨ You Selected: {selectedRoom}
                                </div>
                            )}
                            {validationErrors.room && (
                                <span style={{ color: '#dc3545', fontSize: '0.75rem', fontWeight: '600', display: 'block', marginTop: '6px' }}>⚠️ {validationErrors.room}</span>
                            )}
                        </div>
                    ) : (
                    ...
                    <div style={{ backgroundColor: '#F7F7F2', borderRadius: '24px', padding: '24px', border: '1px solid #E8E8E1', marginTop: '25px' }}>
                        <h4 style={{ margin: '0 0 20px 0', fontSize: '0.75rem', color: '#999', textTransform: 'uppercase', letterSpacing: '2px', fontWeight: '800' }}>
                            Your Selection
                        </h4>
